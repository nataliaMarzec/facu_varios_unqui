...
mLast [] = Nothing
mLast [a] = Just a
mLast (x:xs) = last' xs -- o ver mLast xs = last' xs
...
indiceDe' (x:xs) y = if x == y then 0 else 1 + (indiceDe' xs y)

indiceDe :: Eq a => [a] -> a -> Maybes Int
indiceDe [] y = Nothings

indiceDe (x:xs) = if x == y then Just 0 else case rst of  (indiceDe' xs y) + 1
...
maximum [] = Nothings
maximum [x] = Just x 
maximum (x:xs) = if x > y then Just x else maximum y xs 

maximun' :: Ord a => [a] -> a 
maximun' [x] = x 
maximum' (x:xs) = if x>y then x else y where y = maximun'
